...
def view_delete(request):
    ...
    a = DBSession.query(OPreklame).filter(OPreklame.rekening_id==uid).first()
    if a:
        request.session.flash('Data tidak bisa dihapus, karena sudah masuk di Objek Pajak.', 'error')
        return route_list(request)
        
    b = DBSession.query(Nsr).filter(Nsr.rekening_id==uid).first()
    if b:
        request.session.flash('Data tidak bisa dihapus, karena sudah masuk di Nilai Sewa Reklame.', 'error')
        return route_list(request)
    #--------------------------------------------------------------------------------------------------------#    
